...
const app = {
  ...
  async syncState() {
    ...
    try {
      ...
      for (const b of (app.state.bills || [])) {
        const h = (b.head || b.category || '').trim();
        if (h && !b.head) { b.head = h; }
  ...
  ui: {
    ...
    init() {
      ...
      document.getElementById('form-slip-add').addEventListener('submit', async (e) => {
        ...
        try {
          ...
          const selectedVendor = (document.getElementById('slip-vendor')?.value || '').trim();
          if (!selectedVendor) {
            app.ui.showToast('Please select a vendor from the registered Vendor List.', 'warning');
            return;
          }
          const vendorExists = (app.state.vendors || []).some(v => v.name && v.name.trim().toLowerCase() === selectedVendor.toLowerCase());
          if (!vendorExists) {
            app.ui.showToast(`Vendor "${selectedVendor}" is not in the Vendor List. Please create the vendor first.`, 'error');
            return;
          }
      ...
      document.getElementById('form-bill-add').addEventListener('submit', async (e) => {
        ...
        try {
          ...
          const selectedVendor = (document.getElementById('bill-vendor')?.value || '').trim();
          if (!selectedVendor) {
            app.ui.showToast('Please select a vendor from the registered Vendor List.', 'warning');
            return;
          }
          ...
          const selectedHead = (document.getElementById('bill-head')?.value || document.getElementById('bill-category')?.value || '').trim().toUpperCase();
          if (!selectedHead) {
            app.ui.showToast('Please select a Head (e.g. STATIONARY).', 'warning');
            return;
          }
          const headExists = (app.state.heads || []).some(h => h.name && h.name.trim().toUpperCase() === selectedHead);
          if (!headExists) {
            app.ui.showToast(`Head "${selectedHead}" is not in the Head List. Please create the head first.`, 'error');
            return;
          }
      ...
      // Form: Add/Edit Vendor
      const formVendorAdd = document.getElementById('form-vendor-add');
      if (formVendorAdd) {
        formVendorAdd.addEventListener('submit', async (e) => {
          e.preventDefault();
          const vendorData = {
      ...
      // Form: Add/Edit Head
      const formHeadAdd = document.getElementById('form-head-add');
      if (formHeadAdd) {
        formHeadAdd.addEventListener('submit', async (e) => {
          e.preventDefault();
          const headData = {
            id: document.getElementById('edit-head-id')?.value,
            name: document.getElementById('head-name')?.value
          };
          await app.heads.saveHead(headData);
        });
      }
      const searchHeads = document.getElementById('search-heads');
      if (searchHeads) searchHeads.addEventListener('input', () => app.heads.renderHeadsTable());
      const billHeadSel = document.getElementById('bill-head');
      if (billHeadSel) billHeadSel.addEventListener('change', () => {
        const hc = document.getElementById('bill-category');
        if (hc) hc.value = billHeadSel.value;
      });
      const convHeadSel = document.getElementById('convert-bill-head');
      if (convHeadSel) convHeadSel.addEventListener('change', () => {
        const cc = document.getElementById('convert-bill-category');
      ...
      document.getElementById('report-select-type').addEventListener('change', () => {
        ...
        if (vGrp) {
          vGrp.style.display = document.getElementById('report-select-type').value === 'vendor' ? 'block' : 'none';
        }
        app.reports.renderReportView();
      });
      const reportVendorSelEl = document.getElementById('report-vendor-select');
      if (reportVendorSelEl) {
        reportVendorSelEl.addEventListener('change', () => app.reports.renderReportView());
      }
    ...
    openBillModal(type){
      ...
      if (window.syncBillSegUI) window.syncBillSegUI(targetType);
    },
    openModal(dialogId) {
    ...
    closeModal(dialogId) {
      ...
      if (dialog) {
        ...
        if (dialogId === 'dialog-advance-add') {
          ...
          const slipVendorEl = document.getElementById('slip-vendor');
          if (slipVendorEl) slipVendorEl.value = '';
        } else if (dialogId === 'dialog-bill-add') {
          document.getElementById('edit-bill-id').value = '';
          document.getElementById('dialog-bill-title').innerText = 'Add Direct Bill';
          app.attachments.clearStagedFile('bill');
          const passedType = (arguments.length > 1 && arguments[1]) ? arguments[1] : (document.getElementById('bill-exp-type')?.value || 'advance');
          if (window.syncBillSegUI) window.syncBillSegUI(passedType);
          const billTokenEl = document.getElementById('bill-token');
          if (billTokenEl) billTokenEl.value = '';
          const billBadge = document.getElementById('bill-token-badge');
          if (billBadge) billBadge.innerText = 'AUTO';
          if (app.vendors) app.vendors.populateVendorDropdowns();
          const billVendorEl = document.getElementById('bill-vendor');
          if (billVendorEl) billVendorEl.value = '';
          if (app.heads) app.heads.populateHeadDropdowns();
          const billHeadEl = document.getElementById('bill-head');
          if (billHeadEl) billHeadEl.value = '';
        } else if (dialogId === 'dialog-transfer-add') {
          ...
          app.attachments.clearStagedFile('convert');
        } else if (dialogId === 'dialog-head-add') {
          document.getElementById('edit-head-id').value = '';
          document.getElementById('dialog-head-title').innerText = 'Add New Head';
        }
      }
    },
    ...
    async initiateEdit(storeName, id) {
      try {
        ...
        if (storeName === 'advance_cash') {
          ...
          if (app.vendors) app.vendors.populateVendorDropdowns(record.vendor);
          const sVendor = document.getElementById('slip-vendor');
          if (sVendor) sVendor.value = record.vendor || '';
          ...
          if (app.vendors) app.vendors.populateVendorDropdowns(record.vendor);
          const bVendor = document.getElementById('bill-vendor');
          if (bVendor) bVendor.value = record.vendor || '';
          ...
          if (document.getElementById('bill-category')) document.getElementById('bill-category').value = record.category || record.head || '';
          if (app.heads) app.heads.populateHeadDropdowns(record.head || record.category || '');
          const bHead = document.getElementById('bill-head');
          if (bHead) bHead.value = (record.head || record.category || '').trim().toUpperCase();
    ...
    renderAll() {
      ...
      // Dashboard Muhasib section: Muhasib Bill Sayer total (cleared amount + count)
      try {
        const clearedList = (app.state.bills || []).filter(b => b.expenseType === 'advance' && b.status === 'adv_cleared');
        const clearedCount = clearedList.length;
        setSafeText('dash-advance-cleared-total', app.ui.formatCurrency(app.state.advanceClearedAmount || 0));
        setSafeText('dash-advance-cleared-count', `${clearedCount} bill${clearedCount !== 1 ? 's' : ''} • Click to open register`);
      } catch (e) { /* dashboard cleared card optional */ }

      // Dashboard Hospital section: Hospital Advance Slip + Hospital Bill Sayer totals
      setSafeText('dash-hospital-slips-total', app.ui.formatCurrency(app.state.hospitalSlipsPendingAmount || 0));
      setSafeText('dash-hospital-slips-count', `${app.state.hospitalSlipsPending || 0} slip${(app.state.hospitalSlipsPending || 0) !== 1 ? 's' : ''} • Click to open register`);
      try {
        const hospClearedList = (app.state.bills || []).filter(b => String(b.expenseType || '').toLowerCase().trim() === 'hospital' && b.status === 'hosp_cleared');
        const hospClearedCount = hospClearedList.length;
        setSafeText('dash-hospital-cleared-total', app.ui.formatCurrency(app.state.hospitalClearedAmount || 0));
        setSafeText('dash-hospital-cleared-count', `${hospClearedCount} bill${hospClearedCount !== 1 ? 's' : ''} • Click to open register`);
      } catch (e) { /* dashboard hospital cleared card optional */ }
    ...
    async clearAdvanceBill(id){
      const bill=app.state.bills.find(b=>b.id===id);
      if(!bill){ app.ui.showToast('Bill not found','error'); return; }
      if(bill.status==='adv_cleared'){ app.ui.showToast('Bill already cleared.','warning'); return; }
      app.ui.selectedAdvanceBills = new Set([id]);
      app.ui.updateAdvanceBatchBar();
      app.ui.openBatchClearModal();
    },
    toggleAdvanceBillSelect(id, checked){
      if(!app.ui.selectedAdvanceBills) app.ui.selectedAdvanceBills = new Set();
      if(checked) app.ui.selectedAdvanceBills.add(id);
      else app.ui.selectedAdvanceBills.delete(id);
      app.ui.updateAdvanceBatchBar();
      try{ if(app.mobile && app.mobile.renderAdvanceBillsCards) app.mobile.renderAdvanceBillsCards(); }catch(e){}
    },
    toggleSelectAllAdvanceBills(checked){
      if(!app.ui.selectedAdvanceBills) app.ui.selectedAdvanceBills = new Set();
      const rows = app.state.bills.filter(b=>String(b.expenseType||'').toLowerCase().trim()==='advance' && b.status!=='adv_cleared');
      if(checked){ rows.forEach(b=>app.ui.selectedAdvanceBills.add(b.id)); }
      else { app.ui.selectedAdvanceBills.clear(); }
      app.ui.renderAdvanceBillsTable();
      try{ if(app.mobile && app.mobile.renderAdvanceBillsCards) app.mobile.renderAdvanceBillsCards(); }catch(e){}
      app.ui.updateAdvanceBatchBar();
    },
    clearAdvanceBillSelection(){
      if(app.ui.selectedAdvanceBills) app.ui.selectedAdvanceBills.clear();
      const inline=document.getElementById('advance-batch-name-inline'); if(inline) inline.value='';
      app.ui.renderAdvanceBillsTable();
      try{ if(app.mobile && app.mobile.renderAdvanceBillsCards) app.mobile.renderAdvanceBillsCards(); }catch(e){}
      app.ui.updateAdvanceBatchBar();
    },
    getSelectedAdvanceBills(){
      if(!app.ui.selectedAdvanceBills) app.ui.selectedAdvanceBills = new Set();
      const ids=[...app.ui.selectedAdvanceBills];
      return ids.map(id=>app.state.bills.find(b=>b.id===id)).filter(b=>b && String(b.expenseType||'').toLowerCase().trim()==='advance' && b.status!=='adv_cleared');
    },
    updateAdvanceBatchBar(){
      const sel=app.ui.getSelectedAdvanceBills();
      const n=sel.length;
      const total=sel.reduce((s,b)=>s+(Number(b.amount)||0),0);
      const cEl=document.getElementById('selected-advance-count');
      if(cEl) cEl.textContent = n ? `${n} selected` : '0 selected';
      const tEl=document.getElementById('selected-advance-total');
      if(tEl) tEl.textContent = n ? `• ${app.ui.formatCurrency(total)}` : '';
      const btn=document.getElementById('btn-batch-clear');
      if(btn) btn.disabled = !n;
      ['select-all-advance-bills','select-all-advance-bills-head'].forEach(hid=>{
        const h=document.getElementById(hid); if(!h) return;
        const all=app.state.bills.filter(b=>String(b.expenseType||'').toLowerCase().trim()==='advance' && b.status!=='adv_cleared');
        h.checked = all.length>0 && n>0 && n>=all.length;
        h.indeterminate = n>0 && n<all.length;
      });
    },
    suggestBatchName(){
      const existing=[...new Set((app.state.bills||[]).map(b=>b.clearBatch).filter(Boolean))];
      const dl=document.getElementById('advance-batch-list'); if(dl) dl.innerHTML=existing.map(b=>`<option value="${app.ui.escapeHTML(b)}">`).join('');
      const today=new Date().toISOString().split('T')[0];
      const nextN=existing.length+1;
      return `Batch-${String(nextN).padStart(2,'0')} / ${today}`;
    },
    openBatchClearModal(singleId){
      ...
      const cEl=document.getElementById('batch-clear-count'); if(cEl) cEl.textContent=sel.length;
      const tEl=document.getElementById('batch-clear-total'); if(tEl) tEl.textContent=app.ui.formatCurrency(sel.reduce((s,b)=>s+(Number(b.amount)||0),0));
      const lEl=document.getElementById('batch-clear-list');
      if(lEl) lEl.innerHTML=sel.map(b=>`<div class="batch-clear-item"><span class="batch-clear-bill">#${app.ui.escapeHTML(String(b.billNumber||'-'))} • ${app.ui.escapeHTML(b.vendor||'-')}</span><span class="batch-clear-amt">${app.ui.formatCurrency(b.amount)}</span><button type="button" class="batch-clear-remove" title="Remove" onclick="app.ui.toggleAdvanceBillSelect(${b.id},false);app.ui.renderAdvanceBillsTable();app.ui.openBatchClearModal()">✕</button></div>`).join('');
      app.ui.suggestBatchName();
      app.ui.openModal('dialog-batch-clear');
      setTimeout(()=>{ const ni=document.getElementById('batch-clear-name'); if(ni){ ni.focus(); ni.select(); } },120);
    },
    ...
    async undoAdvanceClear(id){
      ...
      app.ui.showConfirm('Undo Clear', `Move Bill #${bill.billNumber} (${app.ui.formatCurrency(bill.amount)}) back to Muhasib Bills?`, async()=>{
        try{
          ...
          bill.clearedAt=null;
          await app.db.put('bills',bill.id,bill);
          app.ui.showToast('Bill moved back to Muhasib Bills.');
          app.syncState();
  ...
  vendors: {
    ...
    openAddVendorModal(initialName = '', returnContext = null) {
      ...
      if (form) form.reset();
      const editId = document.getElementById('edit-vendor-id');
      if (editId) editId.value = '';
      const title = document.getElementById('dialog-vendor-title');
      if (title) title.innerText = 'Add New Vendor';
    ...
    async saveVendor(formData) {
      ...
      if (editId) {
        const existing = (app.state.vendors || []).find(v => v.id === editId);
        ...
        await app.db.put('vendors', editId, updated);
      ...
      app.ui.closeModal('dialog-vendor-add');
      app.vendors.populateVendorDropdowns(name);
      app.vendors.renderVendorsTable();

      if (app.vendors._returnContext === 'bill') {
        const billVendorSel = document.getElementById('bill-vendor');
        if (billVendorSel) {
          billVendorSel.value = name;
          billVendorSel.dispatchEvent(new Event('change'));
        }
      } else if (app.vendors._returnContext === 'slip') {
        const slipVendorSel = document.getElementById('slip-vendor');
        if (slipVendorSel) {
    ...
    async deleteVendor(id) {
      ...
      const linkedSlips = (app.state.temporarySlips || []).filter(s => (s.vendor || '').trim().toLowerCase() === vName);
      const totalCount = linkedBills.length + linkedSlips.length;

      let msg = `Are you sure you want to delete vendor "${vendor.name}"?`;
      if (totalCount > 0) {
        msg += `\n\nNotice: This vendor has ${totalCount} recorded transaction(s) (${linkedBills.length} bills, ${linkedSlips.length} slips). Existing transactions will retain this vendor name, but creating future entries will require registering the vendor again.`;
      }
      ...
      try {
        ...
        console.error(err);
        app.ui.showToast('Failed to delete vendor.', 'error');
      }
    },
    ...
    renderVendorsTable() {
      const tbody = document.getElementById('list-vendors');
      if (!tbody) return;
      ...
      let totalHospVol = 0;
      let totalMuhasibVol = 0;
      let totalSpendAll = 0;

      list.forEach(v => {
        const norm = (v.name || '').trim().toLowerCase();
        const hospBills = (app.state.bills || []).filter(b => b.expenseType === 'hospital' && (b.vendor || '').trim().toLowerCase() === norm);
        const muBills = (app.state.bills || []).filter(b => b.expenseType === 'advance' && (b.vendor || '').trim().toLowerCase() === norm);
        const slips = app.getActiveTemporarySlips().filter(s => (s.vendor || '').trim().toLowerCase() === norm);

        const hospAmt = hospBills.reduce((s, x) => s + (x.amount || 0), 0);
        const muAmt = muBills.reduce((s, x) => s + (x.amount || 0), 0);
        const slipAmt = slips.reduce((s, x) => s + (x.amount || 0), 0);
        const totalBillsCount = hospBills.length + muBills.length;
        const totalBillsAmt = hospAmt + muAmt;
        const totalSpend = totalBillsAmt + slipAmt;

        vendorStats.set(v.id, {
          hospBillsCount: hospBills.length,
          hospAmt,
          muBillsCount: muBills.length,
          muAmt,
        ...
        totalHospVol += hospAmt;
        totalMuhasibVol += muAmt;
        totalSpendAll += totalSpend;
      });

      // Update Header Summary Elements
      const kpiBadge = document.getElementById('kpi-total-vendors-badge');
      if (kpiBadge) kpiBadge.textContent = `Total: ${list.length} Vendors`;
      ...
      // Sorting
      list.sort((a,b) => {
        const statsA = vendorStats.get(a.id) || { totalSpend: 0, totalBillsCount: 0 };
        const statsB = vendorStats.get(b.id) || { totalSpend: 0, totalBillsCount: 0 };
        if (sortVal === 'name_asc') return (a.name || '').localeCompare(b.name || '');
        if (sortVal === 'name_desc') return (b.name || '').localeCompare(a.name || '');
        if (sortVal === 'spend_desc') return statsB.totalSpend - statsA.totalSpend;
        if (sortVal === 'bills_desc') return statsB.totalBillsCount - statsA.totalBillsCount;
        return 0;
      });
      ...
      list.forEach((v, idx) => {
        ...
        tbody.innerHTML += `
          <tr>
            ...
            <td class="num-val text-right">
              ${stats.totalBillsCount ? `<span class="font-bold">${stats.totalBillsCount}</span> <small class="text-muted">(${app.ui.formatCurrency(stats.totalBillsAmt)})</small>` : '<span class="text-muted" style="opacity:0.4">-</span>'}
            </td>
            <td class="num-val text-right">
              ${stats.slipsCount ? `<span class="badge badge-warning" style="font-size:0.72rem;padding:2px 6px;">${stats.slipsCount} slip${stats.slipsCount > 1 ? 's' : ''} (${app.ui.formatCurrency(stats.slipAmt)})</span>` : '<span class="text-muted" style="opacity:0.4">-</span>'}
            </td>
            <td class="num-val text-right font-bold text-success" style="font-size:0.92rem;">
              ${app.ui.formatCurrency(stats.totalSpend)}
            </td>
            <td class="text-center">
    ...
    viewVendorReport(vendorName) {
      app.ui.switchTab('reports');
      const rSel = document.getElementById('report-select-type');
      if (rSel) {
        rSel.value = 'vendor';
        rSel.dispatchEvent(new Event('change'));
      }
      const vSel = document.getElementById('report-vendor-select');
      if (vSel) {
        vSel.value = vendorName;
        vSel.dispatchEvent(new Event('change'));
      }
    },

    resetFilters() {
      const s = document.getElementById('search-vendors');
      if (s) s.value = '';
      const so = document.getElementById('sort-vendors');
  ...
  heads: {
    ...
    openAddHeadModal(initialName = '', returnContext = null) {
      ...
      if (form) form.reset();
      const editId = document.getElementById('edit-head-id');
      if (editId) editId.value = '';
      const title = document.getElementById('dialog-head-title');
      if (title) title.innerText = 'Add New Head';
      const btn = document.getElementById('btn-save-head');
      if (btn) btn.innerText = 'Save Head';
      const nameInput = document.getElementById('head-name');
      if (nameInput) nameInput.value = initialName || '';
      app.ui.openModal('dialog-head-add');
    ...
    async initiateEdit(id) {
      ...
      document.getElementById('dialog-head-title').innerText = 'Edit Head';
      const btn = document.getElementById('btn-save-head');
      if (btn) btn.innerText = 'Update Head';
      document.getElementById('head-name').value = head.name || '';
      app.ui.openModal('dialog-head-add');
    },
    async saveHead(formData) {
      ...
      if (editId) {
        ...
        await app.db.put('heads', editId, updated);
        const idx = app.state.heads.findIndex(h => h.id === editId);
        if (idx !== -1) app.state.heads[idx] = updated;
        if (oldName && oldName.toUpperCase() !== name) {
          for (const b of (app.state.bills || [])) {
            const cur = (b.head || b.category || '').trim();
            if (cur.toUpperCase() === oldName.toUpperCase()) {
              b.head = name; b.category = name;
              try { await app.db.put('bills', b.id, b); } catch (_) {}
            }
          }
        }
        app.ui.showToast(`Head "${name}" updated!`, 'success');
      } else {
      ...
      app.ui.closeModal('dialog-head-add');
      app.heads.populateHeadDropdowns(name);
      app.heads.renderHeadsTable();
      if (app.heads._returnContext === 'bill') {
        const sel = document.getElementById('bill-head');
        if (sel) { sel.value = name; sel.dispatchEvent(new Event('change')); }
        const conv = document.getElementById('convert-bill-head');
        if (conv) conv.value = name;
      }
      app.heads._returnContext = null;
      return true;
    },
    ...
    populateHeadDropdowns(selected = '') {
      ...
      if (billSel) {
        const cur = selected || billSel.value || document.getElementById('bill-category')?.value || '';
        billSel.innerHTML = opts;
        if (cur) billSel.value = cur;
      }
      const convSel = document.getElementById('convert-bill-head');
      if (convSel) {
        const cur2 = convSel.value || '';
    ...
    renderHeadsTable() {
      ...
      list.forEach((h, idx) => {
        ...
        const bills = (app.state.bills||[]).filter(b => ((b.head||b.category||'').trim().toUpperCase() === hn.toUpperCase()));
        const amt = bills.reduce((s,x) => s + (Number(x.amount)||0), 0);
        tbody.innerHTML += `<tr>
          <td class="text-center font-mono text-muted text-xs" style="opacity:0.7;">${idx+1}</td>
  ...
  reports: {
        ...
        hospBills.forEach(b => {
          html += `
            <tr>
              ...
              <td><span class="source-tag">Hospital Bill</span></td>
              <td class="num-val text-right text-muted">-</td>
        ...
        advBills.forEach(b => {
          html += `
            <tr>
              ...
              <td><span class="source-tag">Muhasib Bill</span></td>
              <td class="num-val text-right text-muted">-</td>
              <td class="num-val text-right text-error">-${app.ui.formatCurrency(b.amount)}</td>
            </tr>
          `;
    ...
    renderReportView() {
      ...
      // Toggle vendor selection field visibility
      const vGrp = document.getElementById('report-vendor-group');
      if (vGrp) {
        vGrp.style.display = (type === 'vendor') ? 'block' : 'none';
      }
      ...
      if (type === 'advance') {
        ...
        const selVendor = (document.getElementById('report-vendor-select')?.value || '').trim();

        if (selVendor) {
          titleDisplay.innerText = `Vendor Comprehensive Statement: ${selVendor}`;
          
          const vBills = (app.state.bills || []).filter(b => (b.vendor || '').trim().toLowerCase() === selVendor.toLowerCase());
          const vSlips = app.getActiveTemporarySlips().filter(s => (s.vendor || '').trim().toLowerCase() === selVendor.toLowerCase());
          ...
          vBills.forEach(b => {
            combined.push({
              date: b.date,
              token: b.tokenNumber || '-',
              type: b.expenseType === 'advance' ? 'Muhasib Bill' : 'Hospital Bill',
              isMuhasib: b.expenseType === 'advance',
              docNo: b.billNumber || '-',
          ...
          vSlips.forEach(s => {
            combined.push({
              date: s.date,
              token: s.tokenNumber || '-',
              type: s.expenseType === 'advance' ? 'Muhasib Temp Slip' : 'Hospital Temp Slip',
              isMuhasib: s.expenseType === 'advance',
              docNo: 'Slip #' + (s.tokenNumber || s.id),
          ...
          const filtered = filterByDateRange(combined);
          filtered.sort((a,b) => new Date(a.date) - new Date(b.date));

          const hospBillsAmt = filtered.filter(x => x.isBill && !x.isMuhasib).reduce((s,x)=>s+x.amount, 0);
          const muBillsAmt = filtered.filter(x => x.isBill && x.isMuhasib).reduce((s,x)=>s+x.amount, 0);
          const slipsAmt = filtered.filter(x => !x.isBill).reduce((s,x)=>s+x.amount, 0);
          const grandTotal = filtered.reduce((s,x)=>s+x.amount, 0);

          metaDisplay.innerHTML = `
            <span>Vendor: <strong>${selVendor}</strong></span> &nbsp;•&nbsp; 
            <span>${dateMetaText}</span> &nbsp;•&nbsp; 
            <span>${filtered.length} Total Records</span> &nbsp;•&nbsp; 
            <span class="text-success font-bold">Total Spend: ${app.ui.formatCurrency(grandTotal)}</span>
          `;
          ...
          filtered.forEach(row => {
            const badgeStyle = row.isMuhasib ? 'background:var(--tertiary-light);color:var(--tertiary);border:1px solid rgba(168,85,247,0.3)' : 'background:rgba(16,185,129,0.12);color:var(--accent);border:1px solid rgba(16,185,129,0.3)';
            const attachAvailable = row.attachmentUrl ? 'Yes' : 'No';
            const attachName = row.attachmentUrl ? (row.fileName || 'document') : '-';

            tbody.innerHTML += `
              <tr>
                <td class="num-val">${app.ui.formatDate(row.date)}</td>
                <td><span class="source-tag font-mono" style="font-size:0.72rem;letter-spacing:0.5px">${row.token}</span></td>
                <td><span class="source-tag" style="${badgeStyle}">${row.type}</span></td>
                <td class="font-mono text-xs">${row.docNo}</td>
                <td>
                  ${row.remarks && row.remarks !== '-' ? `<div style="font-weight:600;">${row.remarks}</div>` : '<span class="text-muted">-</span>'}
                </td>
                <td><span class="status-pill ${row.status}">${row.status}</span></td>
                <td class="text-xs font-mono">${attachAvailable === 'Yes' ? `<a href="${row.attachmentUrl}" target="_blank" class="text-accent underline" style="text-decoration:underline;">${attachName}</a>` : '<span class="text-muted">-</span>'}</td>
                <td class="num-val text-right text-error font-bold">-${app.ui.formatCurrency(row.amount)}</td>
              </tr>
            `;
          });

          if (!filtered.length) {
            tbody.innerHTML = `<tr><td colspan="8" class="text-center text-muted" style="padding:2rem;">No transactions recorded for vendor "${selVendor}" in selected date range.</td></tr>`;
          } else {
            tbody.innerHTML += `
              <tr style="background:var(--bg-elevated);border-top:2px solid var(--border-color);font-weight:bold;">
                <td colspan="4" class="text-muted text-xs" style="vertical-align:middle;">
                  Hospital Bills: ${app.ui.formatCurrency(hospBillsAmt)} &nbsp;|&nbsp; Muhasib Bills: ${app.ui.formatCurrency(muBillsAmt)} &nbsp;|&nbsp; Slips: ${app.ui.formatCurrency(slipsAmt)}
                </td>
                <td colspan="3" class="text-right" style="vertical-align:middle;font-size:0.9rem;">TOTAL VENDOR EXPENDITURE:</td>
                <td class="num-val text-right text-error" style="font-size:1.05rem;font-weight:900;">-${app.ui.formatCurrency(grandTotal)}</td>
              </tr>
            `;
          }
        } else {
          ...
          thead.innerHTML = `
            <tr>
              ...
              <th class="text-right">Hospital Bills (₹)</th>
              <th class="text-right">Muhasib Bills (₹)</th>
              <th class="text-right">Active Slips (₹)</th>
              <th class="text-right">Total Combined (₹)</th>
          ...
          sorted.forEach(v => {
            const norm = (v.name || '').trim().toLowerCase();
            const hAmt = filterByDateRange((app.state.bills || []).filter(b => b.expenseType === 'hospital' && (b.vendor||'').trim().toLowerCase() === norm)).reduce((s,x)=>s+x.amount, 0);
            const mAmt = filterByDateRange((app.state.bills || []).filter(b => b.expenseType === 'advance' && (b.vendor||'').trim().toLowerCase() === norm)).reduce((s,x)=>s+x.amount, 0);
            const sAmt = filterByDateRange(app.getActiveTemporarySlips().filter(s => (s.vendor||'').trim().toLowerCase() === norm)).reduce((s,x)=>s+x.amount, 0);
            const tot = hAmt + mAmt + sAmt;
            gHosp += hAmt; gMu += mAmt; gSlips += sAmt; gAll += tot;

            tbody.innerHTML += `
              <tr>
                <td class="font-bold">${v.name}</td>
                <td class="num-val text-right">${hAmt ? app.ui.formatCurrency(hAmt) : '<span class="text-muted">-</span>'}</td>
                <td class="num-val text-right">${mAmt ? app.ui.formatCurrency(mAmt) : '<span class="text-muted">-</span>'}</td>
                <td class="num-val text-right">${sAmt ? app.ui.formatCurrency(sAmt) : '<span class="text-muted">-</span>'}</td>
                <td class="num-val text-right font-bold ${tot ? 'text-success' : 'text-muted'}">${app.ui.formatCurrency(tot)}</td>
                <td class="text-center">
                  <button type="button" class="btn btn-secondary btn-xs" onclick="app.vendors.viewVendorReport('${v.name.replace(/'/g, "\\'")}')">View Details</button>
                </td>
              </tr>
            `;
          });
          ...
          if (!sorted.length) {
            tbody.innerHTML = `<tr><td colspan="6" class="text-center text-muted" style="padding:2rem;">No registered vendors found.</td></tr>`;
          } else {
            tbody.innerHTML += `
              <tr style="background:var(--bg-elevated);border-top:2px solid var(--border-color);font-weight:bold;">
                <td>GRAND TOTAL:</td>
                <td class="num-val text-right">${app.ui.formatCurrency(gHosp)}</td>
                <td class="num-val text-right">${app.ui.formatCurrency(gMu)}</td>
                <td class="num-val text-right">${app.ui.formatCurrency(gSlips)}</td>
                <td class="num-val text-right text-success" style="font-size:1.05rem;font-weight:900;">${app.ui.formatCurrency(gAll)}</td>
                <td></td>
              </tr>
            `;
          }
    ...
    exportToExcel() {
      ...
      if (selType === 'vendor') {
        ...
        if (selVendor) {
          const vBills = (app.state.bills || []).filter(b => (b.vendor || '').trim().toLowerCase() === selVendor.toLowerCase() && inRange(b.date));
          const vSlips = app.getActiveTemporarySlips().filter(s => (s.vendor || '').trim().toLowerCase() === selVendor.toLowerCase() && inRange(s.date));
          const rows = [
            [`Noor Hospital - Vendor Ledger Statement: ${selVendor}`],
            ['Report Date Range', sVal || 'Start', 'to', eVal || 'End'],
            ['Generated On', new Date().toLocaleString('en-IN')],
            [],
            ['Date', 'Token No', 'Entry / Ledger Type', 'Bill / Invoice #', 'Status', 'Remarks', 'Amount (₹)']
          ];
          const combined = [];
          vBills.forEach(b => combined.push({
            date: b.date,
            token: b.tokenNumber || '-',
            type: b.expenseType === 'advance' ? 'Muhasib Purchase Bill' : 'Hospital Purchase Bill',
            docNo: b.billNumber || '-',
            status: b.status || 'pending',
            remarks: b.remarks || '-',
            amount: b.amount
          }));
          vSlips.forEach(s => combined.push({
            date: s.date,
            token: s.tokenNumber || '-',
            type: s.expenseType === 'advance' ? 'Muhasib Temp Slip' : 'Hospital Temp Slip',
            docNo: 'Slip #' + (s.tokenNumber || s.id),
          ...
          combined.forEach(r => {
            rows.push([app.ui.formatDate(r.date), r.token, r.type, r.docNo, r.status, r.remarks, r.amount]);
          });
          if (!combined.length) rows.push(['No transactions recorded in selected range', '', '', '', '', '', '']);
          const totalAmt = combined.reduce((s, r) => s + (r.amount || 0), 0);
          rows.push([]);
          rows.push(['GRAND TOTAL', '', '', '', '', '', totalAmt]);
          const ws = XLSX.utils.aoa_to_sheet(rows);
          ws['!cols'] = [{wch:13},{wch:14},{wch:24},{wch:18},{wch:14},{wch:30},{wch:16}];
          ws['!merges'] = [{s:{r:0,c:0},e:{r:0,c:6}}];
          XLSX.utils.book_append_sheet(wbSingle, ws, 'Vendor Statement');
          XLSX.writeFile(wbSingle, `NoorHospital_Vendor_${selVendor.replace(/[^a-z0-9]/gi, '_')}_Statement.xlsx`);
          app.ui.showToast(`Vendor statement for "${selVendor}" exported.`);
        } else {
          const rows = [
            ['Noor Hospital - Master Vendor Directory & Financial Summary'],
            ['Generated On', new Date().toLocaleString('en-IN')],
            [],
          ...
          app.vendors.getUniqueVendors().forEach(v => {
            ...
            const sAmt = app.getActiveTemporarySlips().filter(s => (s.vendor || '').trim().toLowerCase() === vName && inRange(s.date)).reduce((s,x)=>s+x.amount, 0);
            rows.push([v.name, v.phone || '-', hAmt, mAmt, sAmt, hAmt + mAmt + sAmt]);
          });
          const ws = XLSX.utils.aoa_to_sheet(rows);
          ws['!cols'] = [{wch:28},{wch:18},{wch:18},{wch:18},{wch:18},{wch:20}];
          ws['!merges'] = [{s:{r:0,c:0},e:{r:0,c:5}}];
          XLSX.utils.book_append_sheet(wbSingle, ws, 'All Vendors Summary');
          XLSX.writeFile(wbSingle, `NoorHospital_All_Vendors_Summary.xlsx`);
          app.ui.showToast('Master vendor summary exported.');
        }
        return;
      }
      ...
      if(selType==='muhasib_bills' || selType==='hospital_bills' || selType==='bills'){
        ...
        const rows=[ [title], ['Date Range', sVal||'Start','to',eVal||'End'], ['Generated On', new Date().toLocaleString('en-IN')], [], ['Date','Token No','Bill Number','Vendor','Amount','Remarks'] ];
        filterBills.forEach(b=> rows.push([app.ui.formatDate(b.date), b.tokenNumber||'-', b.billNumber, b.vendor, b.amount, b.remarks||'-']));
        ...
        const ws=XLSX.utils.aoa_to_sheet(rows); ws['!cols']=[{wch:13},{wch:14},{wch:16},{wch:20},{wch:14},{wch:30}]; ws['!merges']=[{s:{r:0,c:0},e:{r:0,c:5}}]; ws['A1'].s={font:{bold:true,sz:13,color:{rgb:"1F4E79"}},alignment:{horizontal:"center"}};
      ...
      const slipRows = [
        ...
        [],
        ['Date', 'Vendor / Person', 'Amount (₹)', 'Status', 'Attachment Available', 'Attachment File Name', 'Remarks']
      ];
      app.state.temporarySlips.forEach(s => {
      ...
      const billRows = [
        ['Bills Register'],
        [],
        ['Date', 'Bill Number', 'Vendor Name', 'Head', 'Amount (₹)', 'Attachment Available', 'Attachment File Name', 'Remarks']
      ];
      app.state.bills.forEach(b => {
      ...
      app.state.bills.filter(b => b.expenseType === 'advance').forEach(b => drCrCombined.push({ date: b.date, particulars: `${b.vendor || '-'}${b.billNumber ? ' ('+b.billNumber+')' : ''}`, vType: 'Advance Bill', dr: 0, cr: b.amount }));
    ...
    exportPage(page){
      ...
      if(page==='advance'){
        ...
        rows=[['Hospital Temporary Slips (Filtered)'],['Export Date',new Date().toLocaleString('en-IN')],['Total',d.reduce((s,e)=>s+e.amount,0),`Records: ${d.length}`],[],['Date','Token No','Vendor','Amount (₹)','Status','Remarks']];
        d.forEach(e=>rows.push([app.ui.formatDate(e.date),e.tokenNumber||'-',e.vendor,e.amount,e.status,e.remarks||'-']));
        ...
        rows=[['Muhasib Temporary Slips (Filtered)'],['Export Date',new Date().toLocaleString('en-IN')],['Total',d.reduce((s,e)=>s+e.amount,0),`Records: ${d.length}`],[],['Date','Token No','Vendor','Amount (₹)','Status','Remarks']];
        d.forEach(e=>rows.push([app.ui.formatDate(e.date),e.tokenNumber||'-',e.vendor,e.amount,e.status,e.remarks||'-']));
        sheetName='Muhasib Temp Slips';
      } else if(page==='advance-bills'){
        const d=filtered(app.state.bills,'advance-bills').filter(b=>b.expenseType==='advance' && b.status!=='adv_cleared');
        rows=[['Muhasib Bills (Advance) (Filtered)'],['Export Date',new Date().toLocaleString('en-IN')],['Total',d.reduce((s,e)=>s+e.amount,0),`Records: ${d.length}`],[],['Date','Token No','Bill No','Vendor','Head','Amount (₹)','Remarks']];
...
<tfoot><tr><td colspan="4" style="text-align:right">Total</td><td class="r">\u20B9${fmt(total)}</td></tr></tfoot></table>
    ...
    exportBalanceSheetExcel() {
      ...
      hospBills.forEach(b => {
        rows.push(['PART II: OUTFLOWS', `Hospital Bill #${b.billNumber || '-'}: ${b.vendor || '-'}`, 'Hospital Bill', '', b.amount || 0, '']);
      });
      ...
      advBills.forEach(b => {
        rows.push(['PART II: OUTFLOWS', `Muhasib Bill #${b.billNumber || '-'}: ${b.vendor || '-'}`, 'Muhasib Bill', '', b.amount || 0, '']);
      });
  ...
  mobile: {
    ...
    renderAllMobileCards(forceAll = false) {
      if (!forceAll && app.mobile.isMobile()) {
        ...
        } else if (activeId === 'panel-advance-cleared') {
    ...
    renderAdvanceBillsCards() {
      ...
      container.innerHTML = filtered.map(bill=>{
        ...
        return `<div class="mobile-record-card ${app.ui.selectedAdvanceBills && app.ui.selectedAdvanceBills.has(bill.id)?'card-selected':''}" style="border-left:3px solid var(--tertiary)"><div style="display:flex;align-items:center;gap:0.6rem;padding:0.45rem 0.7rem;background:var(--bg-app);border-bottom:1px solid var(--border-color);border-radius:8px 8px 0 0"><label style="display:flex;align-items:center;gap:0.5rem;font-size:0.82rem;font-weight:600;cursor:pointer" onclick="event.stopPropagation()"><input type="checkbox" style="width:18px;height:18px;accent-color:#8b5cf6" ${app.ui.selectedAdvanceBills && app.ui.selectedAdvanceBills.has(bill.id)?'checked':''} onchange="app.ui.toggleAdvanceBillSelect(${bill.id},this.checked);app.ui.renderAdvanceBillsTable()"> Select for Batch</label><button type="button" class="btn btn-secondary btn-sm" style="margin-left:auto" onclick="app.ui.openBatchClearModal(${bill.id})">Move →</button></div><div class="mobile-card-header"><div style="min-width:0;flex:1"><div class="mobile-card-title" style="white-space:normal;word-break:break-word">${app.ui.escapeHTML(bill.vendor)}</div><div class="mobile-card-date">#${app.ui.escapeHTML(bill.billNumber)} • ${app.ui.formatDate(bill.date)}</div></div><div class="mobile-card-amount outflow" style="font-size:1rem">-${app.ui.formatCurrency(bill.amount)}</div></div><div class="mobile-card-meta" style="gap:0.4rem">${bill.tokenNumber ? `<span class="source-tag font-mono" style="font-size:0.7rem;letter-spacing:0.5px">${app.ui.escapeHTML(bill.tokenNumber)}</span>` : ''}<span class="source-tag">${note}</span>${attachmentHtml}</div><div style="display:flex;flex-direction:column;gap:0.35rem;background:var(--bg-app);border:1px solid var(--border-color);border-radius:8px;padding:0.6rem 0.7rem"><div class="mobile-card-row"><span class="mobile-card-label">Date</span><span class="mobile-card-val">${app.ui.formatDate(bill.date)}</span></div><div class="mobile-card-row"><span class="mobile-card-label">Token No</span><span class="mobile-card-val" style="font-size:0.8rem;font-family:monospace">${app.ui.escapeHTML(bill.tokenNumber || '-')}</span></div><div class="mobile-card-row"><span class="mobile-card-label">Bill No</span><span class="mobile-card-val" style="font-size:0.8rem">${app.ui.escapeHTML(bill.billNumber)}</span></div><div class="mobile-card-row"><span class="mobile-card-label">Vendor</span><span class="mobile-card-val" style="font-size:0.8rem;white-space:normal;text-align:right;max-width:55%">${app.ui.escapeHTML(bill.vendor)}</span></div><div class="mobile-card-row"><span class="mobile-card-label">Head</span><span class="mobile-card-val" style="font-size:0.8rem;white-space:normal;text-align:right;max-width:55%">${app.ui.escapeHTML(bill.head||bill.category||'-')}</span></div><div class="mobile-card-row"><span class="mobile-card-label">Amount</span><span class="mobile-card-val" style="color:var(--error)">${app.ui.formatCurrency(bill.amount)}</span></div>${bill.remarks?`<div style="border-top:1px dashed var(--border-color);padding-top:0.35rem;margin-top:0.15rem"><span class="mobile-card-label">Remarks</span><div style="font-size:0.8rem;color:var(--text-muted);margin-top:2px;white-space:normal;word-break:break-word">${app.ui.escapeHTML(bill.remarks)}</div></div>`:''}</div><div class="mobile-card-footer" style="flex-wrap:wrap"><button class="btn btn-secondary btn-sm" onclick="app.ui.clearAdvanceBill(${bill.id})" style="flex:1;border-color:rgba(16,185,129,0.4);color:var(--success);font-weight:700">✓ Clear</button><button class="btn btn-secondary btn-sm" onclick="app.ui.convertBill(${bill.id})" style="flex:1">→ Hosp</button><button class="btn btn-secondary btn-sm btn-edit-action" onclick="app.ui.initiateEdit('bills', ${bill.id})" style="flex:1">Edit</button><button class="btn btn-secondary btn-sm text-error" onclick="app.db.promptDelete('bills', ${bill.id})" style="flex:1">Delete</button></div></div>`;
      }).join('');
    },
    renderAdvanceClearedCards() {
      const container=document.getElementById('mobile-list-advance-cleared');
      if(!container) return;
